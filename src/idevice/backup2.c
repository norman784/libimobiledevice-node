...
static void print_progress_real(FILE *stream_progress, double progress, int flush)
{
    int i = 0;
    fprintf(stream_progress, "\r[");
    for(i = 0; i < 50; i++) {
        if(i < progress / 2) {
            fprintf(stream_progress, "=");
        } else {
            fprintf(stream_progress, " ");
        }
    }
    fprintf(stream_progress, "] %3.0f%%", progress);
...
static void print_progress(node_progress_callback callback, uint64_t current, uint64_t total)
{
    ...
    print_progress_real(stream_progress, (double)progress, 0);
    
    format_size = string_format_size(current);
    fprintf(stream_progress, " (%s", format_size);
    free(format_size);
...
static int mb2_handle_send_file(FILE *stream_err, FILE *stream_out, mobilebackup2_client_t mobilebackup2, const char *backup_dir, const char *path, plist_t *errplist)
{
    ...
    if (_stati64(localfile, &fst) < 0)
        ...
        {
            if (errno != ENOENT)
                fprintf(stream_err, "%s: stat failed on '%s': %d\n", __func__, localfile, errno);
            errcode = errno;
            goto leave;
        }
    ...
    if (!f) {
        fprintf(stream_err, "%s: Error opening local file '%s': %d\n", __func__, localfile, errno);
        errcode = errno;
        goto leave;
    }
...
static void mb2_handle_send_files(FILE *stream_err, FILE *stream_out, mobilebackup2_client_t mobilebackup2, plist_t message, const char *backup_dir)
{
    uint32_t cnt;
    uint32_t i = 0;
    uint32_t sent;
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (mb2_handle_send_file(stream_err, stream_out, mobilebackup2, backup_dir, str, &errplist) < 0) {
            free(str);
...
static int mb2_handle_receive_files(FILE *stream_error, FILE *stream_out, node_progress_callback progress_callback, mobilebackup2_client_t mobilebackup2, plist_t message, const char *backup_dir)
{
    ...
    do {
        ...
        if (r != 4) {
            fprintf(stream_error, "ERROR: %s: could not receive code length!\n", __func__);
            break;
        }
        nlen = be32toh(nlen);
        ...
        if (r != 1) {
            fprintf(stream_error, "ERROR: %s: could not receive code!\n", __func__);
            break;
        }
        ...
        if (f) {
            ...
            fprintf(stream_error, "Error opening '%s' for writing: %s\n", bname, strerror(errno));
        }
        if (nlen == 0) {
            break;
        }
...
void idevice_backup2(struct idevice_backup2_options options, FILE *stream_err, FILE *stream_out, node_progress_callback progress_callback)
{
    ...
    if (!strcmp(command, "backup")) {
        ...
        if (options.encryption.password != NULL) {
            if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
                newpw = strdup(options.encryption.password);
    ...
    /* verify options */
    if (cmd == -1) {
        fprintf(stream_err, "No command specified.\n");
        return;
    }
    ...
    if (cmd == CMD_CHANGEPW || cmd == CMD_CLOUD) {
        ...
        if (stat(backup_directory, &st) != 0) {
            fprintf(stream_err, "ERROR: Backup directory \"%s\" does not exist!\n", backup_directory);
            return;
        }
    }
    ...
    if (cmd == CMD_CHANGEPW) {
        if (!interactive_mode && !backup_password && !newpw) {
            idevice_free(device);
            fprintf(stream_err, "ERROR: Can't get password input in non-interactive mode. Either pass password(s) on the command line, or enable interactive mode with -i or --interactive.\n");
            return;
        }
    ...
    if (cmd != CMD_CLOUD && is_encrypted) {
        ...
        if (backup_password == NULL) {
            ...
            if (!backup_password || (strlen(backup_password) == 0)) {
                ...
                idevice_free(device);
                if (cmd == CMD_RESTORE) {
                    fprintf(stream_err, "ERROR: a backup password is required to restore an encrypted backup. Cannot continue.\n");
                } else if (cmd == CMD_UNBACK) {
                    fprintf(stream_err, "ERROR: a backup password is required to unback an encrypted backup. Cannot continue.\n");
                }
                return;
            }
        }
    ...
    checkpoint:
        ...
        switch(cmd) {
            case CMD_CLOUD:
                ...
                opts = NULL;
                if (err != MOBILEBACKUP2_E_SUCCESS) {
                    fprintf(stream_err, "Error setting cloud backup state on device, error code %d\n", err);
                    cmd = CMD_LEAVE;
                }
                break;
            case CMD_BACKUP:
                ...
                if (err == MOBILEBACKUP2_E_SUCCESS) {
                    ...
                    if (err == MOBILEBACKUP2_E_BAD_VERSION) {
                        fprintf(stream_err, "ERROR: Could not start backup process: backup protocol version mismatch!\n");
                    } else if (err == MOBILEBACKUP2_E_REPLY_NOT_OK) {
                        fprintf(stream_err, "ERROR: Could not start backup process: device refused to start the backup process.\n");
                    } else {
                        fprintf(stream_err, "ERROR: Could not start backup process: unspecified error occured\n");
                    }
                    cmd = CMD_LEAVE;
                }
                break;
            case CMD_RESTORE:
                ...
                if (!mb2_status_check_snapshot_state(backup_directory, source_udid, "finished")) {
                    fprintf(stream_err, "ERROR: Cannot ensure we restore from a successful backup. Aborting.\n");
                    cmd = CMD_LEAVE;
                    break;
                }
                ...
                if (err != MOBILEBACKUP2_E_SUCCESS) {
                    if (err == MOBILEBACKUP2_E_BAD_VERSION) {
                        fprintf(stream_err, "ERROR: Could not start restore process: backup protocol version mismatch!\n");
                    } else if (err == MOBILEBACKUP2_E_REPLY_NOT_OK) {
                        fprintf(stream_err, "ERROR: Could not start restore process: device refused to start the restore process.\n");
                    } else {
                        fprintf(stream_err, "ERROR: Could not start restore process: unspecified error occured\n");
                    }
                    cmd = CMD_LEAVE;
                }
                break;
            ...
            case CMD_UNBACK:
                ...
                if (err != MOBILEBACKUP2_E_SUCCESS) {
                    fprintf(stream_err, "Error requesting unback operation from device, error code %d\n", err);
                    cmd = CMD_LEAVE;
                }
                break;
            case CMD_CHANGEPW:
                ...
                if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
                    if (!willEncrypt) {
                        ...
                        if (!newpw) {
                            fprintf(stream_err, "No backup password given. Aborting.\n");
                        }
                    } else {
        ...
        if (cmd != CMD_LEAVE) {
            ...
            do {
                ...
                if (!strcmp(dlmsg, "DLMessageDownloadFiles")) {
                    ...
                    mb2_handle_send_files(stream_err, stream_out, mobilebackup2, message, backup_directory);
                    ...
                    if (iter) {
                            ...
                            if (key && (plist_get_node_type(val) == PLIST_STRING)) {
                                ...
                                if (str) {
                                    ...
                                    if (rename(oldpath, newpath) < 0) {
                                        fprintf(stream_err, "Renameing '%s' to '%s' failed: %s (%d)\n", oldpath, newpath, strerror(errno), errno);
                                        errcode = errno_to_device_error(errno);
                                        errdesc = strerror(errno);
                                        break;
                                    }
                        ...
                        fprintf(stream_err, "Could not create dict iterator\n");
                    }
                    ...
                    if (err != MOBILEBACKUP2_E_SUCCESS) {
                        fprintf(stream_err, "Could not send status response, error %d\n", err);
                    }
                    ...
                    for (ii = 0; ii < cnt; ii++) {
                        ...
                        if (plist_get_node_type(val) == PLIST_STRING) {
                            ...
                            if (str) {
                                ...
                                if (res != 0 && res != ENOENT) {
                                    if (!suppress_warning)
                                        fprintf(stream_err, "Could not remove '%s': %s (%d)\n", newpath, strerror(res), res);
                    ...
                    if (plist_get_node_type(node_tmp) != PLIST_DICT) {
                        fprintf(stream_err, "Unknown message received!\n");
                    }
                    plist_t nn;
                    int error_code = -1;
                    ...
                    if (error_code != 0) {
                        if (str) {
                            fprintf(stream_err, "ErrorCode %d: %s\n", error_code, str);
                        } else {
