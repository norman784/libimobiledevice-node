...
/**
 * Idevicepair error —Invalid wifi option.
 * @class
 * @constructor
 * @public
 * @property {string} name - Error name.
 * @property {number} code - Error code.
 */
class PairInvalidWifiOptionError extends Error {
    /**
     * 
     * @param {string} message 
     */
    constructor(message) {
        super(message);
        this.name = 'PairInvalidWifiOptionError';
        this.code = native_pair_errors.PAIR_E_INVALID_WIFI_OPTION;
    }
}
...
const getPairParameters = (command, options, callback) => {
	...
	if(options) {
        ...
        pOptions.wifioption = options.wifioption || null;
	}
	...
	child.on('message', res => {
        ...
        switch(true) {
            ...
            case (pairError === native_pair_errors.PAIR_E_SUCCESS
                ...
                callback(null, res.data);
                break;
            // SUCCESS: pair, validate, unpair, wifi commands
            case (pairError === native_pair_errors.PAIR_E_SUCCESS 
                && ideviceError === native_idevice_errors.IDEVICE_E_SUCCESS 
            ...
            case (pairError === native_pair_errors.PAIR_E_INVALID_COMMAND 
                ...
                break;
            case (pairError === native_pair_errors.PAIR_E_INVALID_WIFI_OPTION
                && ideviceError === native_idevice_errors.IDEVICE_E_UNKNOWN_ERROR 
                && lockdownError === native_lockdown_errors.LOCKDOWN_E_UNKNOWN_ERROR):
                callback(new PairInvalidWifiOptionError(errorMessage), null);
                break;
