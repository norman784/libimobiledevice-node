...
const BAKCUP_ERRORS = {
    PURGE_DISK_SPACE: 'ERROR: DLMessagePurgeDiskSpace'
};
...
/**
 * Class error that reports computer disk must purge an amoung of space 
 * determined by spaceToPurge property.
 */
class BackupDiskPurgeError extends Error {
    /**
     * 
     * @param {any} message 
     * @param {number} spaceToPurge // In bytes
     */
    constructor(message, spaceToPurge) {
        super(message);
        this.name = 'BackupDiskPurgeError';
        this.spaceToPurge = spaceToPurge;
    }
}
 ...
 * }} options 
 * @param {(error: Backup2Error |  BackupDiskPurgeError | EncryptionDeviceLockedError | EncryptionInvalidPasswordError | EncryptionAlreadyEnabledError | EncryptionAlreadyDisabledError,
...
exports.idevice_backup2 = function(options, callback, progress) {
    ...
    child.on('message', res => {
        if (res.progress) progress(res.progress);
            ...
            } else if (res.error ? res.error.search(BAKCUP_ERRORS.PURGE_DISK_SPACE) !== -1 : false) {
                const re = /\d+/;
                let spaceToFreed = res.error.match(re) ? res.error.match(re)[0] : '0';
                callback(new BackupDiskPurgeError(res.error, parseInt(spaceToFreed)), null);
            } else {
